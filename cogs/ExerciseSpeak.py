...
class ExerciseSpeak(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.group(invoke_without_command=True, case_insensitive=True, name="pt", aliases=["tt"])
    async def exercise_speak(self, ctx):
        """
        ...
        query = "SELECT content FROM flexbot.messages WHERE author_id=$1 AND guild_id=$2 AND channel_id='505254271017680897' ORDER BY random() LIMIT 20000;"
        try:
            record = await self.bot.pool.fetch(query, userId, ctx.guild.id, timeout=5.0)
        except AttributeError:
            return await ctx.send("Något gick fel i queryn..")
        except Exception as e:
    ...
    def sync_speak(self, ctx, record, userId):
        ...
        variablename = text_model.make_short_sentence(240)
        speech += "{}\n\n".format(variablename)
        return speech


def setup(bot):
    bot.add_cog(ExerciseSpeak(bot))
